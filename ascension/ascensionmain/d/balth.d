...
IF ~~ THEN BEGIN a43 // from: 42.0
  SAY @1091 /* ~I have mastered the power within me and held the evil in check, but I know as well...if not better...then most of my brethren the threat that it holds for all of Faerun.  I will not allow this.  I have dedicated my life to preventing it.~ #74125 */
...
IF ~~ THEN BEGIN a52 // from: 51.0
  SAY @1114 /* ~Very well, then, <CHARNAME>.  I will join you.  You are a worthy <PRO_MANWOMAN>.  Perhaps even worthy of turning Bhaal's dark taint and making something better of it.  I do not know.  But I will take that chance.~ #80549 */
  IF ~~ THEN GOTO a53
END

IF ~~ THEN BEGIN a53 // from: 52.0
  SAY @1115 /* ~Let Melissan tremble when she sees us together, and we shall strike her down as she deserves.  Even if Bhaal himself is resurrected, he will not be able to hold out against us both.  I have preparations to make, <CHARNAME>.  Go to the Throne of Bhaal.  When the time comes, I will be there.~ #80550 */
...
ReputationGT(Player1,17)~ THEN DO ~IncrementGlobal("ConvinceBalth","GLOBAL",1)~ GOTO a67
  IF ~!Alignment(Player1,MASK_GOOD)
!ReputationGT(Player1,17)~ THEN DO ~IncrementGlobal("ConvinceBalth","GLOBAL",-1)~ GOTO a68
END
...
IF ~~ THEN BEGIN a75 // from: 71.1
  ...
  IF ~CheckStatLT(Player1,15,WIS)~ THEN REPLY @1150 /* ~It was told to me by a Solar, an agent of prophecy!  It...oh, I know how it sounds, but it's true!~ #74174 */ DO ~IncrementGlobal("ConvinceBalth","GLOBAL",-1)~ GOTO a76
...
IF ~~ THEN BEGIN a98 // from: 97.0
  SAY @1180 /* ~You mistake the path for the journey, my friend.  Whatever paths you have taken since leaving Candlekeep, the journey has ended here as it was always meant to.  Destiny masters us...we do not master it.~ #74216 */
  IF ~!Alignment(Player1,MASK_EVIL)~ THEN DO ~IncrementGlobal("ConvinceBalth","GLOBAL",1)~ GOTO a44
  IF ~Alignment(Player1,MASK_EVIL)~ THEN DO ~IncrementGlobal("ConvinceBalth","GLOBAL",-1)~ GOTO a94
END
