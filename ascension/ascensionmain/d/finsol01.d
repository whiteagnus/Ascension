...
IF ~False()~ THEN BEGIN a36 // from: BALTH2:7.0
...
IF ~False()~ THEN BEGIN a37 // from: BALTH2:8.0
...
IF ~False()~ THEN BEGIN a38 // from: BALTH2:9.0
  ...
  IF ~InParty("Imoen2")
!StateCheck("Imoen2",4032)~ THEN GOTO a39
  IF ~OR(2)
!InParty("Imoen2")
StateCheck("Imoen2",4032)~ THEN GOTO 5
END
...
IF ~False()~ THEN BEGIN a40 // from: FINBODH:40.0 FINBODH:39.0
  SAY @586 /* ~While it fills me with distaste that you would even bring such a creature of evil before me, god-child, you must remember that in this place an oath is worth more than the words with which it is spoken.  You are no god yet...and while I would rather destroy this undead thing, it now falls on me to enforce the promise you have made.~ #82706 */
